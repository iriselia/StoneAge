...
void LServer_SACheckLock1_recv( char *si, char *Pcdkey, char *Pid, char *flg, char *PASS)
{
	...
	if( !strcmp( flg, "LOCKED") )	{
        saacproto_ACCharList_send( seri , FAILED_STRING , "locked" , Pfd );
		total_ng_charlist++;
	}else	{
...
#define IS_2BYTEWORD( _a_ ) ( (char)(0x80) <= (_a_) && (_a_) <= (char)(0xFF) )
	...
	if( !strcmp( flg, "LOCKED") )	{
        saacproto_ACCharLoad_send( seri , FAILED_STRING , "locked" , Pfd, charindex );
	}else	{
		int lock = 1;
		charindex = getCharIndexByName( Pcdkey , name );
		if( charindex < 0 ){
			saacproto_ACCharLoad_send( seri , FAILED_STRING , "char nonexistent" , Pfd , charindex );
			return;
		}
		log( "loadCharOne: id:[%s] char:[%s]\n", Pcdkey, name );
		if( loadCharOne( Pcdkey , charindex ,  loadbuf ,sizeof( loadbuf ))<0){
			saacproto_ACCharLoad_send( seri , FAILED_STRING ,
		...
		if( lock ){
			...
			if( lockUser( getGSName(seri) , Pcdkey , passwd , 1 , result , sizeof( result ) ,
						   retdata , sizeof( retdata ) , pro , "0")<0){
#endif
				saacproto_ACCharLoad_send( seri , FAILED_STRING , "lock FAIL" , Pfd, charindex );
