...
/*
		...
		このファイルで定義されている関数はぜんぶ認証をつかう。
		
			
				char *c0 : ID
				char *c1 : Passwd  もう認証すんでるから使わない
				char *c2 : Charname
				char *c3 : opt
											...
											char *c2 , char *c3, char *c4, int i0 , int i1 )
{
...
#ifdef _NewSave
		saacproto_ACCharLoad_send( ti , FAILED_STRING , data, mesgid, charindex );
#else
		saacproto_ACCharLoad_send( ti , FAILED_STRING , data, mesgid );
#endif
...
#ifdef _DELAY_UNLOCK
	if( UNlockM_isBe( id ) ){
		saacproto_ACCharLoad_send( ti , FAILED_STRING , "locked" , mesgid, charindex );
		return;
	}else
#endif
    if( isLocked( id ) ){
			...
			saacproto_ACCharLoad_send( ti , FAILED_STRING , "locked" , mesgid, charindex );
#else
			saacproto_ACCharLoad_send( ti , FAILED_STRING , "locked" , mesgid );
#endif
    ...
    if( lock ){
			...
			if( lockUser( getGSName(ti) , id , passwd , 1 , result , sizeof( result ) ,
				retdata , sizeof( retdata ) , process , deadline) <0 ){
#endif
				saacproto_ACCharLoad_send( ti , FAILED_STRING, "lock FAIL!!" , mesgid , charindex );
				return;
			}else	{
#ifdef _TIMEOUTKICK
				//
...
#endif
{
	...
	if( makeSaveCharString( savebuf , sizeof( savebuf ) ,
		...
		log("\n ACCharSave:too long  ");
		saacproto_ACCharSave_send( ti , FAILED_STRING , "too long" , mesgid );
	...
	if( charindex < 0 ){
		...
		if( blankind < 0 ){
			...
			saacproto_ACCharSave_send( ti , FAILED_STRING , "char full" ,mesgid);
			return ret;
		} else {
			charindex = blankind;
		}
	...
	if( saveCharOne( id , charindex , savebuf ) < 0 ){
		...
		saacproto_ACCharSave_send( ti , FAILED_STRING , "disk I/O error or a bug", mesgid );
		return ret;
	}
	else
	{
											 ...
											 char *c2 , char *c3 , char *c4 , int i0 , int i1 )
{
	...
	if( auth != 0 ){
		char data[100];
		snprintf( data, sizeof( data ) , "%d" , auth );
		saacproto_ACCharList_send( ti , FAILED_STRING , data , mesgid );
		total_ng_charlist++;
		return;
	}
...
#ifdef _DELAY_UNLOCK
	if( UNlockM_isBe( id ) ){
		saacproto_ACCharList_send( ti , FAILED_STRING , "locked" , mesgid );
		return;
	}else
#endif
    if( isLocked( id ) ){
												...
												char *c4 , int i0 , int i1 )
{
	...
	if( auth != 0 ){
		char data[100];
		snprintf( data, sizeof( data ) , "%d" , auth );
		saacproto_ACCharDelete_send( ti , FAILED_STRING , data, mesgid );
		return;
	}        
	...
	charindex = getCharIndexByName( id , charname );
	if( charindex < 0 ){
		saacproto_ACCharDelete_send( ti , FAILED_STRING , "nochar",  mesgid );
		return;
	}   
	if( unlinkCharFile( id , charindex ) < 0 ){
		saacproto_ACCharDelete_send( ti , FAILED_STRING , "fileI/O" , mesgid);
		return;
	}
#ifdef	__FAMILY
	// CoolFish: Family 2001/6/12
...
static void getCharOptionFromString( char *str , char *out )
{
	...
	for(c=0;;c++){
		...
		if( IS_2BYTEWORD( str[c] ) ){
			c++;
			if( str[c] == '\0' )break;	// その文字がすでにNULLなら終了
			continue;
		}
		...
		if( str[c]== SPACE ){
			...
			for(;;c++){
				...
				if( str[c] == '\0' ){	// 終了だったら NULL 入れて終わる
					out[outc] = '\0';
					break;	
				}
				
				// ここも２バイト文字の１バイト目ならもう一バイトコピーする
				// 仇仇手ㄡ田奶玄妐儂及ㄠ田奶玄杴卅日手丹域田奶玄戊疋□允月
				//----   For 2Byte Code ----
				if( IS_2BYTEWORD( str[c] ) ){
					out[outc++] = str[c++];		// １バイト目コピー
					out[outc++] = str[c];		// ２バイト目コピー
					if( str[c] == '\0' )break;	// その文字がすでにNULLなら終了
					continue;
				}
				
				// 通常文字は１バイトコピー
				out[outc] = str[c];			// まずコピー
				if( str[c] == '\0' )break;	// 終了だったら終わる
															...
															char *nm , char *opt , char *info )
{
...
#endif
	//    }
	/* つなげる。遅いがエラー検出のために一回ワークにいれる。 */
	/* 勾卅仆月﹝贊中互巨仿□腹請及凶戶卞域莢伐□弁卞中木月﹝ */
	...
	/* NULL 文字が最後にあるから0をいれんといかんのだ。 */
	/* NULL 妐儂互瘉詨卞丐月井日0毛中木氏午中井氏及分﹝ */
...
/*
  ...
  キャラのあいているスロットを検索する
		...
		負ならあいていない。0以上ならあきスロットのindex.
		
			
				単にロードできなかったらそれをかえすだけ。
				もっとちゃんとできるやろけども。
...
#endif
{
	...
	if(!id[0]){
		snprintf(result , resultlen , FAILED_STRING);
											...
											char *c2 , char *c3, char *c4, int i0 , int i1 )
{
	...
	if( auth == 0){ // 扣點完成
		log("\n 再次扣點完成->%s->%s  ", id, deadline);
		if( *deadline != '0')
			ChangeMemDeadline(getHash(id) & 0xff, id, deadline);
	}
	else if( auth == 204) { // 略過時限未到的重複扣點
		log("\n 時限未到->%s  ", id);
	}
	else if( auth != 0 ){ // 扣點錯誤 
		int i;
