                                ...
                                int lock,char* opt , int mesgid)
{
...
#ifdef _NewSave
        saacproto_ACCharLoad_send( ti , FAILED_STRING , "not login" , mesgid, charindex );
#else
        saacproto_ACCharLoad_send( ti , FAILED_STRING , "not login" , mesgid );
#endif
        return;
    }
	...
	if( isChangeGalaxyLocked( id) ) {
		log(" 因星系移民被鎖 ");
		saacproto_ACCharLoad_send( ti , FAILED_STRING , "CHANGE_GALAXY" , mesgid, charindex );
		return;
	}
#endif
	...
	if( inUnlockPool(id) ) {
		log(" 已經在UNLOCKPOOL中 ");
		saacproto_ACCharLoad_send( ti , FAILED_STRING ,
...
/*
   ...
   char *pas : ぱすわーど
   char *charname : キャラ名
   char *opt : オプション文字列。 CharListのときにメモとして返す情報
   char *charinfo : ファイルに保存するキャラクタのステータス情報。
   int id : メッセージid
*/
#ifdef _NewSave
...
#endif
{
    ...
    int ret;
    if( !is_game_server_login(ti) ){
        saacproto_ACCharSave_send( ti , FAILED_STRING , "not login" , mesgid );
        return;
    }
#ifdef _NewSave
...
void saacproto_ACCharList_recv( int ti,char* id,char* pas , int mesgid)
{
	...
	if( isChangeGalaxyLocked( id) ) {
		log(" 因星系移民被鎖 ");
		saacproto_ACCharList_send( ti , FAILED_STRING , "CHANGE_GALAXY" , mesgid );
		return;
	}
#endif

    if( !is_game_server_login( ti ) ){
        saacproto_ACCharList_send( ti , FAILED_STRING , "not login" , mesgid );
        return;
    }
    if( addWorkerTask( id , pas , "1" , charListCallback , ti , id , pas , "", buf ,"0",
                       mesgid , 0 ) < 0 ){
        saacproto_ACCharList_send( ti , FAILED_STRING , "server load too high",
                                   mesgid );
        return;
    }
    log( "charlist: %s\n", id );
                                  ...
                                  char* charname , char *option , int mesgid)
{
    ...
    if( !is_game_server_login(ti) ){
        saacproto_ACCharDelete_send( ti , FAILED_STRING , "not login" , mesgid );
        return;
    }
#ifdef _DEATH_FAMILY_LOGIN_CHECK //無法刪除人物資料
	saacproto_ACCharDelete_send( ti , FAILED_STRING , "server load too high",
							mesgid );
	return;
#endif
...
{
	if( !is_game_server_login(ti) ){
		saacproto_ACLock_send( ti , FAILED_STRING , "not login" , mesgid );
		return;
	}
                                         ...
                                         int msgid2 )
{
    ...
    if( r == 0 ){
        ...
        saacproto_DBDeleteEntryString_send( fd,FAILED_STRING,table,key,
                                            msgid,msgid2 );        
    }
}
                                      ...
                                      int msgid2 )
{
    ...
    if( r == 0 ){
        ...
        saacproto_DBGetEntryString_send( fd,FAILED_STRING,"",table,key,
                                         msgid,msgid2 );        
    }
                                   ...
                                   int msgid2)
{
    if( !is_game_server_login(fd) ){
        saacproto_DBUpdateEntryInt_send( fd, FAILED_STRING, "","",msgid, msgid2 );
        return;
    }    
    if( dbUpdateEntryInt( table , key , value, info ) == 0 ){
        ...
        saacproto_DBUpdateEntryInt_send( fd , FAILED_STRING , table,key, msgid, msgid2 );        
    }
}
                                    ...
                                    int msgid2)
{
    ...
    if( !is_game_server_login(fd) ){
        saacproto_DBGetEntryRank_send( fd, FAILED_STRING, 0,0,"","",msgid, msgid2 );
        return;
    }
    ...
    if( rk <0 ){
        saacproto_DBGetEntryRank_send(
            fd, FAILED_STRING, -1, -1, table, key, msgid, msgid2 );
    } else {
...
{
    if( !is_game_server_login(fd) ){
        saacproto_DBDeleteEntryInt_send( fd, FAILED_STRING,"","",msgid, msgid2 );
        return;
    }
    
    if( dbDeleteEntryInt( table, key ) < 0 ){
        saacproto_DBDeleteEntryInt_send(
            fd, FAILED_STRING, table, key, msgid, msgid2 );
    } else {
        saacproto_DBDeleteEntryInt_send(
            fd, SUCCESSFUL, table, key, msgid, msgid2 );
    }
                                ...
                                int msgid2)
{
    ...
    if( !is_game_server_login(fd) ){
        saacproto_DBGetEntryInt_send( fd, FAILED_STRING,-1,"","",msgid, msgid2 );
        return;
    }
    ...
    if( r < 0 ){
        saacproto_DBGetEntryInt_send( fd,
                                      FAILED_STRING, -1, table, key, msgid, msgid2);
    } else {
                                ...
                                int msgid2)
{
    ...
    if( !is_game_server_login(fd) ){
        saacproto_DBGetEntryByRank_send( fd, FAILED_STRING,"","",msgid, msgid2 );
        return;
    }
    ...
    if( r < 0 ){
        saacproto_DBGetEntryByRank_send(
            fd, FAILED_STRING, "", table, msgid, msgid2 );
    } else {
        saacproto_DBGetEntryByRank_send(
                                       ...
                                       int msgid2)
{
    ...
    if( !is_game_server_login(fd) ){
        saacproto_DBGetEntryByCount_send(
            fd, FAILED_STRING,"",table,count_start,msgid, msgid2 );
        return;
    }
    ...
    if( r < 0 ){
        saacproto_DBGetEntryByCount_send( fd, FAILED_STRING, "", table,
...
#endif
	if (r < 0){
		saacproto_ACAddFM_send(fd, FAILED_STRING, r, index, charfdid);
	}else{
...
#endif
	if (r < 0){
		saacproto_ACJoinFM_send(fd, FAILED_STRING, r, charfdid);
	}else{
	...
	char *charname, char *charid, int index, int charfdid)
{
	...
	if (r < 0){
		saacproto_ACLeaveFM_send(fd, FAILED_STRING, r, charfdid);
	}else{
	...
	int index, char *charname, char *charid, int charfdid)
{
	...
	if(result < 0)
	{
		saacproto_ACDelFM_send(fd, FAILED_STRING, charfdid);
		return;
	}
	// shan modify begin
	// original
...
void saacproto_ACShowFMList_recv(int fd)
{
	...
	r =  ACShowFMList(data);
	if (r < 0){
		saacproto_ACShowFMList_send(fd, FAILED_STRING, r, "Nothing");
	}else{
...
void saacproto_ACShowMemberList_recv(int fd, int index)
{
	...
	if (r < 0){
		saacproto_ACShowMemberList_send(fd, FAILED_STRING, index, r,
	...
	int fmindex, int index, int charfdid)
{
	...
	if (r < 0){
		saacproto_ACFMDetail_send(fd, FAILED_STRING, "Nothing", charfdid);
	}else{
...
#endif		
	if (r < 0){
		saacproto_ACMemberJoinFM_send(fd, FAILED_STRING, charfdid);
	}else{
...
#endif
	if (r < 0){
		saacproto_ACMemberLeaveFM_send(fd, FAILED_STRING, charfdid);
	}else{
...
#ifdef _PERSONAL_FAME   // Arminius: 家族個人聲望
	if (r < 0){
		saacproto_ACFMCharLogin_send(fd, FAILED_STRING, r, floor, fmpopular,
	...
	char *charname, char *charid, int charlv, int index,int charfdid)
{
	...
	r =  ACFMCharLogout(index, fmname, fmindex, charname, charid, charlv,
		charfdid);
	if (r < 0){
		saacproto_ACFMCharLogout_send(fd, FAILED_STRING, charfdid);
	}else{
...
void saacproto_ACFMReadMemo_recv(int fd, int index)
{
	...
	if (r < 0){
		saacproto_ACFMReadMemo_send(fd, FAILED_STRING, index, r, dataindex, "Nothing");
	}else{
	...
	char *data, int index)
{
	...
	if (r < 0){
		saacproto_ACFMWriteMemo_send(fd, FAILED_STRING, index);
	}else{
...
void saacproto_ACFMPointList_recv(int fd)
{
        ...
        if (r < 0)
        {
        	saacproto_ACFMPointList_send(fd, FAILED_STRING, "nothing");
        }else{
...
void saacproto_ACFMSetTAX_recv(int fd, int fm_tax, int index, int fmindex)
{
		...
		{			
			...
			saacproto_ACFMSetTAX_send(fd, FAILED_STRING, -1, 999, index);		// 不要怪我，不想再加欄位了，所以設成999
		}else{
			saacproto_ACFMSetTAX_send(fd, FAILED_STRING, -1, -1, index);		// -1 有許會有問題，要測試一下
		} 
	...
	int index, int fmpointindex, int fl, int x, int y, int charfdid)
{
	...
	if (r < 0){
		saacproto_ACSetFMPoint_send(fd, FAILED_STRING, r, charfdid);
	}else{
	...
	char *losefmname, int losefmindex, int loseindex, int village)
{
	...
	if (r < 0){
		saacproto_ACFixFMPoint_send(fd, FAILED_STRING, r);
	}else{
        ...
        char *data, int color)
{
	...
	if (r < 0){
		saacproto_ACFMAnnounce_send(fd, FAILED_STRING, fmname, fmindex, index,
			1, data, color);
	}else{
		extern gmsv gs[MAXCONNECTION];
		for (i = 0; i < MAXCONNECTION; i ++)
...
void saacproto_ACShowTopFMList_recv(int fd, int kindflag)
{
	...
	if (r < 0)	{
		saacproto_ACShowTopFMList_send(fd, FAILED_STRING, kindflag, r, "Nothing");
	}else	{
	...
	int kindflag, char *data1, char *data2, int charindex, int charfdid)
{
	...
	if (r < 0){
		saacproto_ACFixFMData_send(fd, FAILED_STRING, kindflag, data1, data2, charfdid);
	}else{
	...
	char *losefmname, int losefmindex, int loseindex)
{
	...
	if (r < 0){
		saacproto_ACFixFMPK_send(fd, FAILED_STRING, r, winindex,
			loseindex);
	}else{
	...
	char *data, int charfdid)
{
	...
	if (r < 0){
		saacproto_ACGMFixFMData_send(fd, FAILED_STRING, fmname, charfdid);
	}
	else{
	...
	int kindflag, int charfdid)
{
	...
	if (r < 0){
		saacproto_ACGetFMData_send(fd, FAILED_STRING, kindflag, data, charfdid);
	}else{
...
void saacproto_ACCharInsertPoolItem_recv( int fd, char *cdkey, int userindex, int clifdid, char *Pooldataarg)
{
	...
	if( InsertCharPoolItem( cdkey, Pooldataarg, strlen(Pooldataarg)) <= 0 ) {
		...
		saacproto_ACCharSavePoolItem_send( fd , FAILED_STRING , Pooldataarg, clifdid );
	}
}
...
void saacproto_ACCharGetPoolItem_recv( int fd, char *cdkey, int userindex, int clifdid, int npcid)
{
	...
	if( ret == -1 ){//找不到檔案
		saacproto_ACCharGetPoolItem_send( fd , FAILED_STRING, loadbuf, clifdid, npcid);
		return;
	}
...
void saacproto_ACCharInsertPoolPet_recv( int fd, char *cdkey, int userindex, int clifdid, char *Pooldataarg)
{
	...
	if( InsertCharPoolPet( cdkey, Pooldataarg, strlen(Pooldataarg)) <= 0 ) {
		log("\n InsertPoolPet( %s) err!!\n", cdkey);
		saacproto_ACCharSavePoolPet_send( fd , FAILED_STRING , Pooldataarg, clifdid );
	}
}
...
void saacproto_ACCharGetPoolPet_recv( int fd, char *cdkey, int userindex, int clifdid, int npcid)
{
	...
	if( ret == -1 ){//找不到檔案
		saacproto_ACCharGetPoolPet_send( fd , FAILED_STRING, loadbuf, clifdid, npcid);
		return;
	}
