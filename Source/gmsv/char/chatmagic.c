...
void CHAR_CHAT_DEBUG_level( int charaindex, char* message )
{
	...
	sprintf( buf1, "LEVEL 設為%d。", atoi( message ));
...
void CHAR_CHAT_DEBUG_exp( int charaindex, char* message )
{
	...
	sprintf( buf1, "EXP 設為%d。", atoi( message ));
	CHAR_talkToCli( charaindex, -1, buf1, CHAR_COLORYELLOW);
}
...
// shan 2001/12/18 Begin   由於改變過多，故將原先的function註掉
...
// shan 2001/12/15 Begin   由於改變過多，故將原先的function註掉
...
void CHAR_CHAT_DEBUG_setTrans( int charaindex, char *message)
{
	...
	CHAR_talkToCli( charaindex, -1, "設定轉生", CHAR_COLORWHITE);
...
void CHAR_CHAT_DEBUG_waeikick( int charindex, char* message )
{
    ...
    if(find){
       sprintf(kctalk,"華義豬頭工程師施展超無敵豬殺技「豬豬豬豬豬豬豬•••••••把」");
       for(i=0;i<MAX_USER;i++){
           int i_use;
           i_use=CONNECT_getUse(i);
           if(i_use){
     ...
     for(i=0;i<MAX_USER;i++){
         ...
         if(i_use){
            ...
            if(strcmp(cdkey,token)==0){
               fd_charaindex = CONNECT_getCharaindex( i );
               CONNECT_getCharname( i, szName, sizeof( szName ) );
               sprintf(kcmsg,"%s淪入地窖。",szName);
               CHAR_talkToCli(fd_charaindex, -1, "因做壞事而入獄。", CHAR_COLORYELLOW);
      ...
      if(find){
         sprintf(kctalk,"華義大魔王施展無敵風火輪「輪輪輪輪輪輪輪輪••••••••把」");
         for(i=0;i<MAX_USER;i++){
             int i_use;
             i_use=CONNECT_getUse(i);
             if(i_use){
            ...
            if(strcmp(token,cdkey)==0){
               ...
               snprintf( line, sizeof( line),
                         "把 name:%s account=%s 傳送到 FL=%d X=%d Y=%d",
                          szName,cdkey,fl,x,y);
                 CHAR_talkToCli( charindex, -1 , line , CHAR_COLORWHITE );
                 CHAR_talkToCli( fd_charaindex, -1 ,"＊.＊被華義大魔王傳送", CHAR_COLORWHITE );
            }                                  
         }
     }
}
...
{	
	...
	if( delobjnum > 0 ){
		char buf[256];
		sprintf( buf, "清除%d閒置寵物。", delobjnum);
...
void CHAR_CHAT_DEBUG_cleanqute( int charaindex, char *message)
{
	...
	CHAR_talkToCli( charaindex, -1, "沒有找到對象!!", CHAR_COLORRED);
}
#endif
...
void CHAR_CHAT_DEBUG_getyou(int charaindex,char *message)
{
	...
	if(!getnum) CHAR_talkToCli( charaindex, -1, "範圍內沒有人", CHAR_COLORYELLOW);
}
#endif
    ...
    for(k=0;k<MAX_USER;k++){
        ...
        if(i_use){
            ...
            if(strcmp(cdkey,cdkeytoken)==0){
				...
				if(sendnum<=1) CHAR_talkToCli( charaindex, -1, "範圍內沒有別的玩家", CHAR_COLORYELLOW);
