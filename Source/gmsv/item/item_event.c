...
typedef struct {
    char* cmd;        /* 識別文字列 */
    char*   onmessage;    /*  メッセージ文字列    */
    char*   offmessage;    /*  メッセージ文字列    */
    int   element;    /* 対象になる要素 */
    int   maxElement; /* elementで指定された要素の最大値をもつ要素 */
} ITEM_EFFECTPARAM;
...
BOOL ITEM_MedicineInit(ITEM_Item* itm)
{
    ...
    for(p=effectarg; *p != '\0'; ){
        ...
        *q = '\0';
        value = strtol(arg, & q, 10); /* strtol()はOK? */
...
#define RAND(x,y)   ((x-1)+1+ (int)( (double)(y-(x-1))*rand()/(RAND_MAX+1.0)) )
		...
		if( CHAR_getInt( toindex, CHAR_PETID) == 718 
			...
			if( CHAR_getInt( toindex,CHAR_LV ) < 74 )	{
				sprintf(buf1,"給我的嗎？好美麗的項鍊喔！〈能力起了變化〉");
...
void ITEM_useLottery( int charaindex, int toindex, int haveitemindex)
{
	...
	// for debug
#if 0
    snprintf( buff, sizeof( buff), "等賞：%d 出目: %d%d%d%d%d%d",
    			hit,
    			(int)(n[0]+1),(int)(n[1]+1),
    			(int)(n[2]+1),(int)(n[3]+1),
    			(int)(n[4]+1),(int)(n[5]+1)
...
void ITEM_useSkup( int charaindex, int toindex, int haveitemindex)
{
	...
	CHAR_talkToCli(charaindex, -1, "你感受到自己的能力被提昇了。", CHAR_COLORWHITE);
...
void ITEM_useNoenemy( int charaindex, int toindex, int haveitemindex)
{
	...
	setNoenemy(fd);
	CHAR_talkToCli(charaindex, -1, "你感受到週邊的殺氣消失了。", CHAR_COLORWHITE);
	CHAR_DelItem( charaindex, haveitemindex);
...
#endif
	CHAR_talkToCli(charaindex, -1, "你感受到週邊突然充滿了殺氣！", CHAR_COLORYELLOW);
	CHAR_DelItem( charaindex, haveitemindex);
...
void ITEM_AddPRSkillPercent( int charaindex,int toindex,int haveitemindex)
{
    ...
    CHAR_sendCToArroundCharacter( CHAR_getWorkInt( charaindex , CHAR_WORKOBJINDEX ));
	CHAR_talkToCli( charaindex, -1, "所有職業技能熟練度上升１０％。", CHAR_COLORYELLOW);
#endif
}
...
void ITEM_useFusionEditBase( int charaindex, int toindex, int haveitemindex)
{
	int itemindex;
	int work[4]={0,0,0,0};
	int anhour = PETFEEDTIME;//寵物餵食時間(單位：秒)
...
#if 1 // 消化藥
		{
			...
			if( arg != NULL && !strncmp( arg, "消", 2) ) {
				...
				if( (time_l + anhour) <= nowTime ) {
					...
					min = deftime/60;
					sec = deftime%60;
					sprintf( msg, "寵物蛋已經可以餵食了，超過餵食時間%d分%d秒。", min, sec);
					CHAR_talkToCli( charaindex, -1, msg, CHAR_COLORYELLOW);
					return;
				}
			}
...
#endif
		if( (time_l + anhour) > nowTime ){ //現在的時間要超過(time_l+anhour)才可以餵食
			char Mess1[256];
			int min, sec, deftime;
			...
			min = deftime/60;
			sec = deftime%60;
			sprintf( Mess1, "寵物蛋需過%d分%d秒才可餵食。", min, sec);
