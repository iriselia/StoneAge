...
int all_nosee = 0;		/* １の時　全ての敵を no_see に */
int all_nobody = 0;		/* １の時　全ての敵を no_body に */
int one_loop_born = 1;	/* １ループで最高生み出す数 oneloop_born */

/*------------------------------------------------------------
 * 敵をうみだす場所を決定する
 * 引数
 *  cr      NPC_Create*         座標の幅が決っているNPC_Createのポインタ
 *  nobody  int                 1 の時は誰もいない時につくる
 *                                          (誰かいても作る)
 *                              0 の時は誰もいない時は作らない。
 *                                          (誰かいた時のみ作る)
 *  nosee   int                 1 の時はプレイヤーの視界の中でも作る。
 *                              0 の時はプレイヤーの視界の外で作る。
 *  point   NPC_searchPoint*    出現させる場所を代入する
 *  isflying    BOOL            飛んでいるかどうか。飛んでいるなら TRUE で
 *                                  出現位置んののマップとの当り判定
 *                                  をしなくなる
 * 返り値
 *  作れる時は  TRUE
...
void NPC_generateLoop( BOOL checkall )
{
    ...
    int    CreateOk=0;
    int enemynum ;
    static int  createcount = 0;        /* createの何番目から挑戦するか */
