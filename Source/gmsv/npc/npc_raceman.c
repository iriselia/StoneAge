...
void Raceman_SelectWindow( int meindex, int talker, int num,int select)
{
	...
	switch( num) {
		  ...
		  case 3:	//獵寵登記
			...
			ret = Raceman_CheckGameType(meindex,talker);
			if(ret == 1)	
				Raceman_MsgDisp( meindex, talker, 9 , -1 );	//族長身分不合
			else if(ret == 4)
...
void Raceman_MsgDisp(int meindex,int talker,int num ,int select)
{
	...
	if(NPC_Util_GetArgStr( meindex, argstr, sizeof(argstr)) == NULL) {  //讀參數
...
//return 0: OK  1:族長身分不合 2:新手等級不合 3:新手轉身不合 4:非正式家族
int Raceman_CheckGameType(int meindex , int talker )
{
	char argstr[NPC_UTIL_GETARGSTR_BUFSIZE];
	char buf[64];
...
BOOL Raceman_EventAddItem( int meindex,int talker )
{
	...
	if(gmode < 4 )	//限定種類
		sprintf( buf1, "目前獵寵記錄%d隻。", CHAR_getInt( talker,CHAR_CATCHCNT1+gmode-1 ) );
	else{	
		//random一各題目給他
		askno = rand() % asktotal; 
...
BOOL Raceman_checkpet( int meindex, int talker , int select , char *token )
{
	...
	{			
		...
		if( petNo >= 640 ){
			...
			return FALSE;// 出問題囉，玩家拿不能擁有的寵物來登記 
		}
