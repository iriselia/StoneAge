...
BOOL NPC_PlayerCheck(int meindex,int talker)
    ...
    /*-- ポインタの場所を覚えておく??-*/
    Player.ptr = ptr;
    Player.talkerindex = talker;	//npcのインデックス???
	Player.quizno = 0;		//現在の問題数
	Player.answer = 0;		//正解数
	Player.ansno = 0;		//答えの番号を覚えておく（答えをバラバラに出した際に使用)
