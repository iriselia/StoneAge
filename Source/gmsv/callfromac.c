...
/*
 *  ロックの返答。これは解除にしか使わない。現在ほぼ使われる予定はない
 */
...
{	
	...
	if (strcmp(result, SUCCESSFUL) == 0){	
		...
		sprintf(msg,"%s 莊園稅率調整為 %d％",msg, fm_tax);
...
*/
		...
		CHAR_talkToCli(index, -1, "設定莊園稅率失敗，請稍後再試", CHAR_COLORYELLOW);
	}
